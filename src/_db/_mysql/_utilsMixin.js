...
module.exports = function(CONF) {
    ...
    async.series(
      [
        function(callback) {
          ...
          var sql = that.format(that.sqlTemplates["dbUpdCheck0001"], [
            CONF.get("DB_CONNECTION_INDEX_DB")
          ]);
          that.query(sql, function(err, result) {
            if (result.length === 0) {
              // field not found: add it!
              var sql = that.format(that.sqlTemplates["dbUpdPatch0001"], [
                CONF.get("DB_CONNECTION_INDEX_DB")
              ]);
  ...
  this.getHostID = function(host, port, cb) {
    var sql = that.format(that.sqlTemplates["getHostID"], [
  ...
  this.addHost = function(host, port, cb) {
    ...
    that.getHostID(host, port, function(err, host_id) {
      if (!err) {
        ...
        var sql = that.format(that.sqlTemplates["addHost"], [
          CONF.get("DB_CONNECTION_HOST_DB"),
          host,
          port
        ]);
  ...
  this.updateHostStatus = function(hostID, status, cb) {
    var sql = that.format(that.sqlTemplates["updateHostStatus"], [
      CONF.get("DB_CONNECTION_HOST_DB"),
      status,
      hostID
    ]);
  ...
  this.getAvailableHost = function(cb) {
    ...
    var sql =
      that.format(that.sqlTemplates["getHostByStatus"], [
        CONF.get("DB_CONNECTION_HOST_DB"),
        /* Status: un-indexed */ 0
  ...
  this.setOutdatedStatus = function(hostID, page, status, cb) {
    ...
    if (page !== null) {
      // all pages
      sql = that.format(that.sqlTemplates["setOutdatedPage"], [
        CONF.get("DB_CONNECTION_INDEX_DB"),
        status,
        hostID,
        page
      ...
      sql = that.format(that.sqlTemplates["setOutdatedHost"], [
        CONF.get("DB_CONNECTION_INDEX_DB"),
        status,
        hostID
      ]);
  ...
  this.removeOutdatedPages = function(hostID, cb) {
    var sql = that.format(that.sqlTemplates["removeOutdated"], [
      CONF.get("DB_CONNECTION_INDEX_DB"),
      hostID
    ]);
    that.query(sql, function(err) {
  ...
  this.getIndexedPageInfo = function(hostID, page, cb) {
    var sql = that.format(that.sqlTemplates["getPageInfo"], [
      CONF.get("DB_CONNECTION_INDEX_DB"),
      hostID,
      page
    ]);
  ...
  this.indexPage = function(
    ...
    if (
      ...
      extraArgs["response"] &&
      extraArgs["response"]["headers"] &&
      extraArgs["response"]["headers"]["content-type"]
    ) {
      extraArgs["contentType"] =
        extraArgs["response"]["headers"]["content-type"];
    }
    ...
    if (extraArgs.response && extraArgs.response.headers) {
  ...
  this.getPagesMap = function(host, page, cb) {
    ...
    // get links
    uparsedSqlLinks +=
      "select CONCAT(hostname, linkedpage) as url, textlink from " +
    ...
    // get linked by
    uparsedSqlLinks +=
      "select CONCAT(hostname, page) as url, textlink from " +
    ...
    var sql = that.format(uparsedSqlLinks, [
      CONF.get("DB_CONNECTION_HOST_DB"),
      host,
      page,
      host,
