...
module.exports = function() {
  ...
  this.getPage = function(urlObject, interestedPlugins, callback) {
    try {
      var headers = {
        "User-Agent":
          "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.135 Safari/537.36" // that.CONF.get("CRAWLER_NAME") + "/" + that.CONF.VERSION + " (http://www.openwebspider.org)"
      };
      ...
      const library = urlObject["protocol"].includes("https") ? https : http;
          ...
          if (
            ...
            (response.headers["content-length"] &&
              parseInt(response.headers["content-length"]) >
                that.CONF.get("MAX_PAGE_SIZE") * 1000)
          ) {
            req.abort();
            response.req.end();
            ...
            function(ret) {
              ...
              for (var pluginName in ret) {
                if (ret[pluginName][0] === true) {
                  ...
                  ret[pluginName][0] === false &&
                  ret[pluginName][1] &&
                  ret[pluginName][1]["action"] ===
                    that.plugins_conts.PLUGIN_SKIP_PAGE
                ) {
                  skipPage = true;
                  break;
                }
